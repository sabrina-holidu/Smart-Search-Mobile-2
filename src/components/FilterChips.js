...
const FilterChips = ({ keywordCount = 0, guestCount = null, extractedKeywords = [] }) => {
  ...
  // Update filter count when keywords are extracted
  useEffect(() => {
    if (extractedKeywords && extractedKeywords.length > 0) {
      // The extractedKeywords will auto-check filters in FiltersModal
      // Set the count to the number of keywords that will be mapped to filters
      setFilterCount(extractedKeywords.length);
    }
  }, [extractedKeywords]);
  ...
  const handleApplyFilters = (filters) => {
    ...
    // Calculate total number of checked filters
    const count = 
  ...
  const handleApplyDates = (newDates) => {
    setDates(newDates);
    setIsDatesModalOpen(false);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-sm font-medium text-gray-800">{formatDateRange()}</span>
      </div>
